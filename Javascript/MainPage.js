...
function MainPage(appLogic) {
                  ...
                  type: "div", class: "MainPage_ChampionDiv row", elements: [
                    this.championSelectDiv = CreateElement({
                      type: "div", class: "MainPage_ChampionSelectDiv col s10", elements:
                        [
                          this.championSelect = CreateElement({
                            type: "select",
                            id: "championSelect",
                            class: "MainPage_ChampionSelect icons"
                          })
                        ]
                    }),
                    this.skinSelectDiv = CreateElement({
                      type: "button",
                      id: "selectSkin",
                      class: "MainPage_ButtonSelectSkin btn col s2",
                      text: appLogic.translate("selectSkin")
                    }),
                        ...
                        this.modalSkinContent = CreateElement({
                          type: "div", class: "modal-content", elements: [
                            CreateElement({
                              type: "h4",
                              class: "center-align",
                              text: appLogic.translate("selectYourSkin")
                            }),
  ...
  for (var i = 0; i < ChampionList.length; i++) {
    var element = CreateElement({
      type: "option",
      class: "left circle",
      value: ChampionList[i],
      text: ChampionList[i],
      appendTo: this.championSelect
    });
  ...
  for (var i = 0; i < ExtendedChampionsData[this.championSelect.value].skins.length; i++) {
    var element = CreateElement({
      type: "a", id: "skin" + i, class: "carousel-item skin" + (i == 0 ? " MainPage_ActiveSkin" : ""), elements: [
        CreateElement({
          type: "img",
          src: "http://ddragon.leagueoflegends.com/cdn/img/champion/loading/" + this.championSelect.value + "_" + ExtendedChampionsData[this.championSelect.value].skins[i].num + ".jpg"
        }),
        CreateElement({
          type: "div",
          class: "center-align",
          text: ExtendedChampionsData[this.championSelect.value].skins[i].name
        })
  ...
  this.onlineBoxDiv.appendChild(
    CreateElement({
      type: "div", class: "MainPage_OnlinePlayerDiv", elements: [
        CreateElement({
          type: "div",
          class: "MainPage_OnlinePlayerNameDiv",
          text: this.appLogic.translate("onlinePlayers") + ": " + this.appLogic.networkManager.onlinePlayers.length
        }),
...
MainPage.prototype.updateNewsList = function() {
    ...
    CreateElement({
      type: "div", class: "MainPage_OnlinePlayerDiv", elements: [
        CreateElement({
          type: "div",
          class: "MainPage_OnlinePlayerNameDiv",
          text: this.appLogic.translate("news") + ": "
        }),
  ...
  var playerDiv = CreateElement({
    type: "li", class: "MainPage_OnlinePlayerDiv collection-item avatar", elements: [
      CreateElement({
        type: "img",
        src: "http://ddragon.leagueoflegends.com/cdn/4.20.1/img/champion/" + "Gnar" + ".png",
        class: "champion-frame"
      }),
      ...
      CreateElement({
        ...
        text: "We've taken the decision to disable Skins. The goal of this project is to bring back the old League of Legends experience, but not to steal the business to Riot Games. Also, we expect to bring the custom skins support during this month"
      })
    ]
  });
